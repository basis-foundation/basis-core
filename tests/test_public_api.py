...
class TestDeprecatedApiPackage:
    """
    basis_core.api is a deprecated stub scheduled for removal after v0.1.
    It must not be treated as public API. Importing from it must emit
    a DeprecationWarning. EnforcementPoint is re-exported for the
    deprecation window only.
    """

    def test_api_package_emits_deprecation_warning(self) -> None:
        """Importing basis_core.api must emit a DeprecationWarning."""
        import importlib
        import sys
        import warnings

        # Ensure fresh import so the warning fires even if the module is cached.
        for key in list(sys.modules):
            if key == "basis_core.api" or key.startswith("basis_core.api."):
                del sys.modules[key]

        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            importlib.import_module("basis_core.api")

        assert any(
            issubclass(w.category, DeprecationWarning) and "basis_core.api" in str(w.message)
            for w in caught
        ), "Importing basis_core.api must emit a DeprecationWarning."

    def test_api_enforcement_still_importable(self) -> None:
        """The stub must remain importable (with a warning) during the deprecation window."""
        import warnings
