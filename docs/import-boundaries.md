...
The top-level scanners above are non-recursive per subpackage; nested `*/operation_aware/` packages that need the same protection get their own targeted recursive test. `tests/operation_aware/test_vocabulary_boundaries.py` protects `domain/operation_aware_vocabulary.py`, and `tests/test_import_boundaries.py::test_audit_operation_aware_does_not_import_from_policy_enforcement_or_adapters` recursively protects `audit/operation_aware/`. A dedicated recursive guard in `tests/test_import_boundaries.py` (`test_policy_operation_aware_does_not_import_a_forbidden_layer`) verifies that every module under `policy/operation_aware/` remains below the audit, evaluation, enforcement, and adapter layers. `tests/test_import_boundaries.py::test_evaluation_operation_aware_does_not_import_from_adapters_or_enforcement` recursively protects `evaluation/operation_aware/` (first created by PR 26, Milestone 8), asserting it never imports `adapters/` or `enforcement/`; `test_evaluation_does_not_import_from_adapters_or_enforcement` covers the non-recursive top-level `evaluation/` package the same way the other top-level scanners cover their packages.
...
- `enforcement/` is the **runtime orchestration layer**. It is the only component authorized to compose policy/evaluation, decision handling, audit writing, and side effects into a single execution path. No other `basis_core` subpackage may import from it — only application/integration code above the kernel (including downstream repositories such as `basis-gateway`) does. That is by design: `basis_core.enforcement.OperationAwareEnforcementPoint` (constructed via its public `for_bundle(bundle)` factory, per `docs/public-api.md`) is the documented downstream integration point for operation-aware authorization. Downstream repositories construct operation-aware authorization through `OperationAwareEnforcementPoint.for_bundle(PolicyBundle)` and must not import `basis_core.evaluation.*` to do so — the evaluation engine remains internal to `basis-core`, constructed by the factory on the caller's behalf.
